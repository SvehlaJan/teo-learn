...
## Hat / Accessory Pipeline (2026-05-08)

Reference images generated by Codex CLI `$imagegen` skill (gpt-image-2) — 4 views, 1024×1024 PNG, saved to `meshy_output/reference_images/hat_baseball_cap_v1/`.

Meshy task: `019e0871-7410-70b5-b74e-2d8425d0071b` (multi-image-to-3d, 30 credits).
Source GLB: `meshy_output/20260508_183917_hat-baseball-cap-v1_019e0871/model.glb`.

Blender fitting (MCP):
- Imported source (136k verts); applied Decimate to 14,791 verts (ratio 0.06)
- Smooth shading applied
- Baked Meshy texture replaced with clean solid red Principled BSDF (base color `(0.85, 0.15, 0.10)`, roughness 0.85, metallic 0)
- Scale 0.357× applied to match head width (head 0.437m × 1.1 margin = 0.481m)
- Final position (2026-05-27 browser-verified): Z 1.56–1.72 (brim at ~63% head height, crown 2 cm above head top), Y center −0.016 m
- Positioning note: in this avatar's Blender file, **Blender −Y = toward the browser camera** (the character's face side in Three.js). Moving a hat in +Y pushes it toward the back of the head as seen by the viewer; move in −Y to shift toward the face/camera.
- Icosphere artifact: the original Meshy rigged source contained Icosphere objects that propagated into all 9 base GLBs. Stripped from all files on 2026-05-27 via Blender MCP batch pass; `export_plain_avatar_shoes.py` now also strips `Icosphere*` names.
- Exported to `public/avatar/garments/hat_red_cap_v1.glb`

Runtime slot: `accessory`. Attached to `Head` bone via `attachAccessoryToHeadBone` (all roots in the garment scene are parented to `Head` via `Object3D.attach`). Hat is animation-ready (`animationReady: true`) because the Head bone moves as a rigid unit.

Catalog entry: `hat_red_cap_v1` in `AVATAR_ACCESSORY_ITEMS`, `avatarCatalog.ts`.
