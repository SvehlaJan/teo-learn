...
Check if any other file imports `PRAISE_ENTRIES` or `AUDIO_PHRASES` directly from `contentRegistry`:
```bash
grep -r "PRAISE_ENTRIES\|AUDIO_PHRASES\|AUDIO_PHRASE_LIST" /home/skclaw/teo-learn/src --include="*.ts" --include="*.tsx"
```

For any remaining callsites, update them to use `getLocaleContent(locale).praiseEntries` or `getLocaleContent(locale).audioPhrases`.
