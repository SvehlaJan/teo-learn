...
export function SyllablesGame({ settings, onExit, onOpenSettings }: SyllablesGameProps) {
  ...
  return (
    <div className="min-h-screen relative bg-bg-light flex flex-col">
      ...
      <div aria-hidden="true" className="absolute top-1/4 left-4 sm:left-10 w-20 h-20 sm:w-32 sm:h-32 rounded-3xl bg-success opacity-30 -rotate-12 blur-sm pointer-events-none" />
      <div aria-hidden="true" className="absolute bottom-10 right-4 sm:bottom-20 sm:right-20 w-32 h-32 sm:w-48 sm:h-48 rounded-full bg-accent-blue opacity-20 translate-y-10 blur-md pointer-events-none" />
    </div>
  );
}
