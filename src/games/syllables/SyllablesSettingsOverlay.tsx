...
interface SyllablesSettingsOverlayProps {
  settings: GameSettings;
  onUpdate: (settings: GameSettings) => void;
  onClose: () => void;
}

export function SyllablesSettingsOverlay({ settings, onUpdate, onClose }: SyllablesSettingsOverlayProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-bg-light/95 backdrop-blur-md p-4 sm:p-8">
      <div className="w-full max-w-2xl max-h-[90vh] bg-white rounded-[32px] sm:rounded-[40px] shadow-block overflow-hidden border-4 border-white flex flex-col">
        <div className="p-6 sm:p-10 border-b-2 border-shadow/30 bg-bg-light/50 text-center shrink-0">
          ...
          <p className="text-base sm:text-xl opacity-60 font-medium">Hra so slabikami</p>
        </div>
        ...
        <div className="p-4 sm:p-10 space-y-4 sm:space-y-8 overflow-y-auto flex-1">
          ...
          <div className="pt-8 border-t-2 border-shadow/10">
            <h3 className="text-2xl sm:text-3xl font-bold mb-6 text-center">Počet kariet</h3>
            <div className="grid grid-cols-2 gap-4">
              {([4, 6] as const).map((size) => (
                <button
                  key={size}
                  onClick={() => onUpdate({ ...settings, syllablesGridSize: size })}
                  className={`py-4 rounded-2xl font-bold text-xl transition-all ${
                    settings.syllablesGridSize === size
                      ? 'bg-primary text-white shadow-block scale-105'
            ...
            <p className="text-center mt-4 text-lg opacity-50 font-medium">Vyberte počet kariet v hre</p>
          </div>
        </div>
